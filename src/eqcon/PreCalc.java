...
public class PreCalc {
    public double[][] main(double[] yi0, double[] dr, int cas){
       //
       int idx; 
       // set constants
       double e=1435.16; // track gauge 
       double r0=920;    // rolling radii
       // min dr>=0 (dr~0)
       ...
       double dy = 0.01;
       double ini = yi0[0]; double end = yi0[yi0.length-1]; 
       double ip = 1+((end-ini)/dy); int ipi = (int) ip;
       // create tranversal y vector [mm]
       double[] yii = new VecOp().linspace(ini,end, ipi);
       // interpolation of s(y) function
       double[] s = new Minterp().cubic(s0, yi0, yii);
